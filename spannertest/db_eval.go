...
func (ec evalContext) coerceString(target spansql.Expr, slit spansql.StringLiteral) (spansql.Expr, error) {
	...
	case spansql.Numeric:
		rat := new(big.Rat)
		_, ok := rat.SetString(string(slit))
		if !ok {
			return nil, fmt.Errorf("coercing string literal %q to NUMERIC: invalid format", slit)
		}
		return coercedValue{
			val:  rat,
			orig: slit,
...
func compareVals(x, y interface{}) int {
	...
	case *big.Rat:
		// Handle string to NUMERIC comparison
		if s, ok := y.(string); ok {
			yRat := new(big.Rat)
			_, ok := yRat.SetString(s)
			if !ok {
				panic(fmt.Sprintf("bad numeric string %q", s))
			}
			y = yRat
		}
		return x.Cmp(y.(*big.Rat))
	}
}
